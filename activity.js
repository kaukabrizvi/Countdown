...
const removebook = (title, author) => bookList.splice({title: title, author: author});
const listthem = (input) => input==="authors"?bookList.forEach((author)=>console.log(author)):input=="title"?bookList.forEach((title)=>console.log(title)):console.log("invalid input");
...
let count = 0;
  ...
  const truthys = (exampleArray) => array.forEach((val), val==true? count++: count=count); return count;
